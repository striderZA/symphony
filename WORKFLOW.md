...
1. Fetch the issue by explicit ticket ID via `linear_graphql`.
...
5. For `Todo` tickets, do startup sequencing in this exact order:
   - `update_issue(..., state: "In Progress")` via `linear_graphql`
...
- If comment editing is unavailable in-session, report blocked.
